...
void imgui_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if ((key == GLFW_KEY_PRINT_SCREEN) && (action == GLFW_RELEASE))
    {
        ...
        screenshot_thread = std::thread(stbi_write_png, file_name, res_x, res_y, 3, pixel_buffer, 3 * res_x);
    ...
    if ((key == GLFW_KEY_ESCAPE) && (action == GLFW_RELEASE))
    {
        ...
        //===============================================================================================================================================================================================================
        if (imgui_window->imgui_handle_input)
        {
    ...
    if (imgui_window->imgui_active && imgui_window->imgui_handle_input)
    {
        //===============================================================================================================================================================================================================
    ...
    : frame(0), res_x(res_x), res_y(res_y)
{
    ...
    glfwWindowHint(GLFW_AUTO_ICONIFY, GLFW_FALSE);                        /* disable auto-iconification when window loses focus */
    ...
    if (debug_context)
    {
        if (glDebugMessageCallback)
        {
            ...
            debug_msg("Debug mode requested, but glDebugMessageCallback API is not available.");
    }
    ...
    //===================================================================================================================================================================================================================
    // print current versions of the libraries used
    //===================================================================================================================================================================================================================
    /*
    ...
    */
    ...
    mouse_wheel = mouse_delta = mouse = glm::dvec2(0.0);
    ...
    ImGui::CreateContext();
    ImGuiIO& io = ImGui::GetIO();
    io.BackendFlags |= ImGuiBackendFlags_HasMouseCursors;           // honor GetMouseCursor() values (optional)
    io.BackendFlags |= ImGuiBackendFlags_HasSetMousePos;            // honor io.WantSetMousePos requests (optional, rarely used)
    ...
    // load cursors (revert to arrow cursor for ResizeAll, ResizeNESW, ResizeNWSE)
    // FIXME when exposes these them
...
void imgui_window_t::new_frame()
{
    ...
    if (imgui_active)
    {
        ...
        //  - mouse position in screen coordinates (set to -inf,-inf if no mouse / window is not in focus)
        ...
        if (imgui_handle_input)
        {
            ...
            io.MouseWheelH += (float) mouse_wheel.x;
            io.MouseWheel += (float) mouse_wheel.y;
            mouse_wheel = glm::dvec2(0.0);
        }

        if (imgui_handle_input && !(io.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange) &&
...
void imgui_window_t::set_ui_style()
{
...
imgui_window_t::~imgui_window_t()
{
    ...
    for (ImGuiMouseCursor cursor_n = 0; cursor_n < ImGuiMouseCursor_Count_; ++cursor_n)
        glfwDestroyCursor(mouse_cursors[cursor_n]);
